...
const FormUsername = () => {
    const { setUsername, errorCredentials, setErrorCredentials } = useLoginFormContext();
...
const FormPassword = (props: { onKeyDown: Function }) => {
    const { setPassword, errorCredentials, setErrorCredentials, errorMessage } = useLoginFormContext();
    function updateValue(value: string) {
        setPassword(value);
        setErrorCredentials(false);
    }
    return (
        <div className="mb-3">
            ...
            <input
                ...
                required
            />
            <div id="passwordInputFeedback" className={`invalid-feedback ${styles.error}`}>
